...
def home():
    ...
    if current_user.is_authenticated:
        if current_user.is_admin:
            ...
            notjoinedroom = [[r.name, r.description, r.timestamp.strftime('%Y-%m-%d'), r.owner, len(User_Has_Room.query.filter_by(room_id=r.id, status=1).all()),'Yes' if r.isShow else 'No',r.id] for r in notjoinedroom if r.room_type==0]
            ...
            for r in joinedroom:
                ...
                mainroom.append([unread,r.name, r.description, r.timestamp.strftime('%Y-%m-%d'), r.owner, len(User_Has_Room.query.filter_by(room_id=r.id, status=1).all()),'Yes' if r.isShow else 'No',r.id])
            ...
            waitroom = [[r.name, r.description, r.timestamp.strftime('%Y-%m-%d'), r.owner, len(r.users),'Yes' if r.isShow else 'No'] for r in
                        waitroom]

            return render_template('chat/user_home.html', user_amount=user_amount,joinedroom=mainroom,notjoinedroom=notjoinedroom,waitroom=waitroom)
    else:
        return render_template('auth/login.html')
...
def startchat(room_name):
    ...
    room_id = room.id
    isShow = room.isShow
    return render_template('chat/room_message.html', messages=messages[::-1],current_user=current_user,room_id=room_id,room_name=room_name,type=0,isShow=int(isShow))
...
def privatechat(user_id):
    ...
    if r:
        ...
        room=Room(name=current_user.nickname,first_owner_id=user_id,second_owner_id=current_user.id,room_type=1,isShow=1)
        db.session.add(room)
        db.session.commit()
        ...
        userhasroom1 = User_Has_Room(user_id=user_id, room_id=room.id,  status=1, user=User.query.filter_by(id=user_id).first(),room=room)
    ...
    return render_template('chat/room_message.html', messages=messages[::-1],current_user=current_user,room_id=room_id,room_name=room_name,type=1,isShow=isShow)
